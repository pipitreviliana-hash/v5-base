export const run = {
   ...
   async: async (m, {
      ...
      if (command == 'prefix') {
         ...
         if (args[0].length > 1 && !Utils.getEmojis(args[0])) return client.reply(m.chat, Utils.texted('bold', `🚩 Enter only 1 prefix.`), m)
         ...
         if (!args || !args[0]) return client.reply(m.chat, Utils.example(isPrefix, command, '#'), m)
         if (args[0].length > 1 && !Utils.getEmojis(args[0])) return client.reply(m.chat, Utils.texted('bold', `🚩 Enter only 1 prefix.`), m)
         if (system.prefix.lenght < 2) return client.reply(m.chat, Utils.texted('bold', `🚩 Can't removing more prefix.`), m)
