export const run = {
   ...
   async: async (m, {
      ...
      if (command == '+cmdstic') {
         ...
         if (typeof sticker[hash] != 'undefined') return client.reply(m.chat, `${Utils.texted('bold', `🚩 Sticker is already in the database with text / command`)} : ${Utils.texted('monospace', sticker[hash].text)}`, m)
         sticker[hash] = {
            text: text,
            created: new Date() * 1
         }
         ...
         let hash = m.quoted.fileSha256.toString().replace(/,/g, '')
         if (typeof sticker[hash] == 'undefined') return client.reply(m.chat, Utils.texted('bold', `🚩 Sticker is not in the database.`), m)
         delete sticker[hash]
