...
export const run = {
   ...
   async: async (m, {
      ...
      try {
         if (!text) return client.reply(m.chat, Utils.example(isPrefix, command, 'to | subject | message'), m)
         let q = m.quoted ? m.quoted : m
         let mime = (q.msg || q).mimetype || ''
         client.sendReact(m.chat, '🕒', m.key)
         let [to, subject, msg] = text.split`|`
         if (!to || !subject || !msg) return client.reply(m.chat, Utils.example(isPrefix, command, 'to | subject | message'), m)
         ...
         if (!mime) {
            ...
            let json = await Utils.getFile(await q.download())
            const mailOptions = {
               from: {
